...
request.get('http://127.0.0.1:33649/server/list',function(err,res,body){
  ...
  body = JSON.stringify(JSON.parse(body).map(function (server) {
    delete server.domain
    delete server.id
    delete server.starting
    delete server.stopping
    server.path = typeof server.path !== 'string' ? null : server.path
  ...
  request.put('http://127.0.0.1:33649/close',function(err2,res2,body2){
    clearTimeout(timeout);
